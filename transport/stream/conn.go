// Package stream implements the Meshtastic client API stream protocol.
// This protocol is used to send and receive protobuf messages over serial or TCP connections.
// See https://meshtastic.org/docs/development/device/client-api#streaming-version for details.
package stream
...
// Conn implements the meshtastic client API stream protocol.
...
// NewClientConn creates a new Conn for a client connecting to a radio.
// It sends a wake message to wake the radio if it's sleeping.
func NewClientConn(conn io.ReadWriteCloser) (*Conn, error) {
	sConn := &Conn{conn: conn}
...
// NewRadioConn creates a new Conn for a radio accepting connections.
// Unlike NewClientConn, it does not send a wake message.
func NewRadioConn(conn io.ReadWriteCloser) *Conn {
...
func (c *Conn) Write(in proto.Message) error {
